  ...
  func addLocalMonitorForEvents() {
    localEventMonitor = NSEvent.addLocalMonitorForEvents(matching: [.keyDown, .flagsChanged]) { [weak self] event in
      ...
      if let self {
        ...
        if event.keyCode == .kVK_ANSI_T, event.userModifierFlags == [.shift, .command] {
          self.toggleWorkspaceSidebar(.tags)
          return nil
        }

        if event.keyCode == .kVK_ANSI_H, event.userModifierFlags == [.shift, .command] {
          self.showWorkspaceHub()
          return nil
        }
        ...
        if event.keyCode == .kVK_ANSI_V, event.userModifierFlags == [.option, .command] {
